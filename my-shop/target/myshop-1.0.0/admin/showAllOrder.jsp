		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<table id="tb_list" class="table table-striped table-hover table-bordered table-condensed">
					...
					<c:forEach items="${orderList}" var="map" varStatus="i">
					<tr>
						<td>${i.count}</td>
						<td>${map.order.oid}</td>
						...
						<td>
							...
							<c:if test="${map.order.state eq 2}">
								已支付,待发货
							</c:if>
							<c:if test="${map.order.state eq 3}">
								已发货,待收货
							</c:if>
							<c:if test="${map.order.state eq 4}">
								订单完成
							</c:if>
						</td>
						...
						<td>
							<c:if test="${map.order.state eq 2}">
								<button type="button" class="btn btn-danger btn-sm" onclick="sendOrder('${map.order.oid}')">发货</button>
							</c:if>
						</td>
					</tr>
					</c:forEach>
